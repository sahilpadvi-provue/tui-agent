...
describe("shimmer", () => {
  test("one span per character", () => {
    expect(shimmer("abc", 0, dark.shimmer)).toHaveLength(3);
  });
  test("the character under the head is the brightest", () => {
    ...
    const spans = shimmer("abc", 3 + 1, dark.shimmer);
    expect(spans[1]?.color).toEqual(rgb("#ffffff"));
  });
  test("no phase of the shimmer is flat", () => {
    ...
    for (let phase = 0; phase < 64; phase++) {
      const shades = new Set(shimmer(word, phase, dark.shimmer).map((s) => s.color));
  ...
  test("the highlight arrives and leaves rather than popping", () => {
    ...
    const brightestAt = (phase: number) =>
      shimmer(word, phase, dark.shimmer).findIndex((s) => JSON.stringify(s.color) === JSON.stringify(rgb("#ffffff")));
  ...
  test("every character takes the highlight once per cycle", () => {
    ...
    for (let phase = 0; phase < 13; phase++) {
      const i = shimmer(word, phase, dark.shimmer).findIndex((s) => JSON.stringify(s.color) === JSON.stringify(rgb("#ffffff")));
      if (i !== -1) seen.add(i);
    }
    expect(seen.size).toBe(word.length);
  });
  test("characters keep their order and text", () => {
    expect(shimmer("hi", 0, dark.shimmer).map((s) => s.text)).toEqual(["h", "i"]);
  });
});

describe("highlightCommand", () => {
  const text = (cmd: string) => highlightCommand(cmd, dark).map((s) => s.text).join("");
  ...
  test("the binary is tinted, once", () => {
    const spans = highlightCommand("npm test", dark);
    // Asserted as the role, not the colour: what matters is that the binary
      // is a name, and the theme decides what a name looks like.
      expect(spans[0]).toEqual({ text: "npm", color: dark.name });
  });
  test("a path is tinted like the binary, because both are names", () => {
    const spans = highlightCommand("ls -la src/x", dark);
    const binary = spans.find((s) => s.text === "ls");
    const path = spans.find((s) => s.text === "src/x");
    expect(path?.color).toBe(binary?.color);
    expect(path?.color).toEqual(dark.name);
  });
  ...
  test("an operator is chrome, not a colour of its own", () => {
    const spans = highlightCommand("cat a | wc -l", dark);
